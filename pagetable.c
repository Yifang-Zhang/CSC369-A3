...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		frame = evict_fcn();
	    pgtbl_entry_t *table_ent= coremap[frame].pte;

		// All frames were in use, so victim frame must hold some page
		// Write victim page to swap, if needed, and update pagetable
		// IMPLEMENTATION NEEDED
        
	    if(table_ent->frame & PG_DIRTY){
	        /*get frame no of the page*/
	    	unsigned int frame_no = table_ent->frame >> PAGE_SHIFT;
	   
	   	    if((table_ent->swap_off = swap_pageout( frame_no , table_ent->swap_off)) == INVALID_SWAP){
	    		/* could not store page in memory */
	    		exit(1);
	    	}

	    	table_ent->frame |= PG_ONSWAP;
            evict_dirty_count++;
	    	/* Update pagetable and clear old entry */
	    	
        }else{
            evict_clean_count++;

        }
        table_ent->frame  &= ~PG_VALID;
        table_ent->frame  &= ~PG_DIRTY;
...
char *find_physpage(addr_t vaddr, char type) {
	...
	// IMPLEMENTATION NEEDED
	//initialize page table if it doesn’t exist
	if (pgdir[idx].pde == 0){
    ...
    unsigned t_idx = PGTBL_INDEX(vaddr);
    p = &p_table[t_idx];
    p->vaddr = vaddr; 

	// Check if p is valid or not, on swap or not, and handle appropriately
    if((p->frame & PG_VALID) == 0){ 
        // if not valid then its not in memory
    	if ( p->frame & PG_ONSWAP){
            //allocate virtual frame for page in virtual mem
    		int frame = allocate_frame(p);
            //get page info and put in mem at new frame no 
            assert (swap_pagein(frame, p->swap_off) == 0);
            // set new PFN so the physical address is corresponding to right page entry
            p->frame = frame << PAGE_SHIFT;
            //set to not onswap
            p->frame &= ~PG_ONSWAP;
			   // just swapped in, make not dirty. not sure is still counts as modified
			   // p->frame &= ~PG_DIRTY;
    	...
    	miss_count++; //got a miss  	

    }else{
     // page is in memory and a hit is found
     	hit_count++; 
